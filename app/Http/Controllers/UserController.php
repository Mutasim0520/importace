...
use App\Gbp as GBP;
use Session;
use App\Request as requsted_item;
...
class UserController extends Controller
{
    ...
    public function subscriber (Request $request){
     ...
     public function showAccountSettingsPage(){
        $requested_item = requsted_item::where(['user_id' => Auth::user()->id])->orderBy('id','DESC')->get();
        $Catagory = Catagory::all();
        $Districts = explode(',',Storage::disk('public')->get('districts.txt')) ;
        return view('User.accountSettings',['requested_item' => $requested_item,'User'=>Auth::user(),'Catagory'=>$Catagory , 'Districts'=>$Districts]) ;
    }
